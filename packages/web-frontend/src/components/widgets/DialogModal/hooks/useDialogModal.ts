...
import {
  dialogModalReducer,
  initialState,
} from '@/components/widgets/DialogModal/redusers/dialogModalReducer';
import {
  CompanionsDialogModalProps,
  DialogModal,
} from '@/components/widgets/DialogModal/types/DialogModal';
...
export const useDialogModal = ({ receiver_ids, resetCards }: CompanionsDialogModalProps) => {
  ...
  const receivers =
    state.invitationCompanion?.id !== undefined ? state.invitationCompanion?.id : receiver_ids;
