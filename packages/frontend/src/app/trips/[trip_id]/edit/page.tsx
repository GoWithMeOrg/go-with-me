...
import type { ITrip } from '@/app/types/Trip';
...
const TripEditPage: NextPage<PageProps> = () => {
    ...
    return (
        <div className="TripEditPage">
            <h3>Edit Trip {(data as { trip: { name: string } })?.trip?.name}</h3>
            {loading && <p>Loading...</p>}
            {error && <p>Error : {error.message}</p>}
            {!error && (data as { trip: ITrip })?.trip && (
                <TripForm tripData={(data as { trip: ITrip })?.trip} onSubmit={handleEdit} />
            )}
        </div>
    );
};
